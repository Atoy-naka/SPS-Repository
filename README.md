...
## 🧩 その他の機能・工夫
プロフィールごとの投稿一覧表示
自分のプロフィール画面では自身の投稿一覧が、他ユーザーのプロフィールではそのユーザーの投稿一覧が表示されるように実装。ユーザーごとの投稿アーカイブが一目で確認できます。

コミュニティの自動削除
コミュニティから最後のメンバーが退会した際、自動的にコミュニティ自体も削除される仕様にすることで、不要なデータの蓄積を防ぎ、データベースをクリーンに保ちます。

UI・UXへの配慮
投稿後はリロードせずとも即時に画面に反映される簡易的なリアルタイム処理を実装（Ajax的な体験）。ユーザーがスムーズに操作できるよう工夫しています。
